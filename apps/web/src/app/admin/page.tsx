...
// Marcador oficial de un partido para la vista admin: 90' + nota de penales/TE.
...
export default function AdminDashboard() {
  ...
  // Vista de pronósticos (carga bajo demanda; un payload, dos modos de filtro).
  ...
  async function loadPredictions() {
    setBusy("preds"); setError("")
    try {
      const d = await apiFetch<PredData>("/api/admin/predictions")
      setPredData(d)
      if (d.matches.length && !predMatchId) setPredMatchId(d.matches[0].id)
      if (d.participants.length && !predUserId) setPredUserId(d.participants[0].id)
    } catch (e) { setError((e as Error).message) } finally { setBusy("") }
  }
  ...
  return (
    <div className="container mx-auto p-4 md:p-8 min-h-screen">
      ...
      {/* Pronósticos de participantes (solo admin): un payload, dos filtros. */}
      <section className="mb-8">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
          <h2 className="section-bar headline text-xl">Pronósticos de participantes</h2>
          {predData && (
            <div className="flex gap-1 bg-card border border-border rounded-lg p-1">
              <button onClick={() => setPredMode("match")}
                className={predMode === "match"
              ...
              <button onClick={() => setPredMode("participant")}
                className={predMode === "participant"
                  ? "text-xs font-black uppercase px-3 py-1.5 rounded bg-primary text-primary-foreground"
                  : "text-xs font-black uppercase px-3 py-1.5 rounded text-muted-foreground hover:text-foreground"}>
                Por participante
              </button>
        ...
        {!predData ? (
          <div className="bg-card border border-border rounded-lg p-5 text-center">
            <p className="text-sm text-muted-foreground mb-3">Consulta el marcador que pronosticó cada participante, filtrando por partido o por persona.</p>
            <button onClick={loadPredictions} disabled={busy === "preds"}
              className="bg-secondary text-secondary-foreground py-2 px-5 rounded font-black uppercase text-sm hover:bg-secondary/80 disabled:opacity-50">
              {busy === "preds" ? "Cargando..." : "Cargar pronósticos"}
            </button>
          </div>
        ) : (() => {
          const data = predData!
          const predIndex = new Map<string, PredRow>()
          data.predictions.forEach((p) => predIndex.set(`${p.userId}|${p.matchId}`, p))
